...
> **Status:** This community maintained project checks streams twice each month. Availability can change between checks, and 1 channels in the main playlist are currently being rechecked during the six week grace period.
...
Use the **Smart TV safe** playlist when a channel displays one frame and then stops, or when a television reports an HLS manifest error. It contains the 133 channels whose public streams avoid manifest shapes known to break limited players.
...
> **وضعیت پروژه:** این پروژه به‌صورت داوطلبانه نگهداری می‌شود و لینک‌های پخش ماهی دو بار بررسی می‌شوند. ممکن است وضعیت یک شبکه در فاصله میان دو بررسی تغییر کند. در حال حاضر، ۱ شبکه از پلی‌لیست اصلی دوباره بررسی می‌شوند و تا شش هفته برای بازگشت آن‌ها صبر می‌کنیم.
...
اگر تصویر یک شبکه فقط برای لحظه‌ای نمایش داده می‌شود، یا تلویزیون خطای مانیفست HLS نشان می‌دهد، **نسخه سازگار با تلویزیون هوشمند** را امتحان کنید. این پلی‌لیست شامل ۱۳۳ شبکه است که ساختار استریم آن‌ها روی پخش‌کننده‌های محدود بهتر کار می‌کند.
