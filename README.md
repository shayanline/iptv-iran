...
IPTV Iran provides 201 curated Iranian and Persian language television channels as remote M3U playlists. Add a playlist URL to a compatible player, with no project account or project software to install.

[![Refresh playlists](https://github.com/shayanline/iptv-iran/actions/workflows/refresh.yml/badge.svg)](https://github.com/shayanline/iptv-iran/actions/workflows/refresh.yml) ![Channels](https://img.shields.io/badge/channels-201-1f6feb) ![Checked](https://img.shields.io/badge/last%20checked-15%20September%202026-2da44e) [![License: MIT](https://img.shields.io/github/license/shayanline/iptv-iran)](LICENSE)

> **Status:** This community maintained project checks streams twice each month. Availability can change between checks, and 3 channels in the main playlist are currently being rechecked during the six week grace period.
...
|:--|:--|--:|:--|
| **Main playlist** | You want one entry for every channel. | 201 channels | [Open the main playlist](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/iran.m3u) |
| **With backups** | You want alternate entries for a channel. | 624 streams | [Open the backup playlist](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/iran-all-streams.m3u) |
| **Smart TV safe** | Your player shows one frame and then stops. | 128 channels | [Open the compatibility playlist](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/iran-compat.m3u) |

The **Main playlist** contains every channel, including 3 channels that are being rechecked during the grace period. Entries marked `[IR]` require an Iranian connection.
...
| Satellite · News & Current Affairs | 29 | [Open playlist](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/categories/sat-news.m3u) |
...
| Satellite · Factual, Culture & Lifestyle | 5 | [Open playlist](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/categories/sat-documentary.m3u) |
...
| Religious · Christian | 10 | [Open playlist](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/categories/religious-christian.m3u) |
...
Each entry can include a measured `tvg-quality` value of `SD`, `HD`, `FHD`, or `4K`. The current channel mix is 1 4K, 56 FHD, 51 HD, 87 SD, 6 unknown.
...
Use the **Smart TV safe** playlist when a channel displays one frame and then stops, or when a television reports an HLS manifest error. It contains the 128 channels whose public streams avoid manifest shapes known to break limited players.
...
The current publication contains 201 channels in 13 categories from 624 tracked working or grace period streams. Read the [English channel catalogue](CHANNELS.md) or the [Persian channel catalogue](CHANNELS.fa.md).
...
این پروژه ۲۰۱ شبکه تلویزیونی ایرانی و فارسی‌زبان را در قالب پلی‌لیست‌های M3U ارائه می‌کند. لینک یکی از پلی‌لیست‌ها را در یک برنامه پخش سازگار وارد کنید تا شبکه‌ها در دسترس شما قرار بگیرند. استفاده از این پلی‌لیست‌ها به حساب کاربری یا نصب نرم‌افزار جداگانه‌ای از طرف پروژه نیاز ندارد.

> **وضعیت پروژه:** این پروژه به‌صورت داوطلبانه نگهداری می‌شود و لینک‌های پخش ماهی دو بار بررسی می‌شوند. ممکن است وضعیت یک شبکه در فاصله میان دو بررسی تغییر کند. در حال حاضر، ۳ شبکه از پلی‌لیست اصلی دوباره بررسی می‌شوند و تا شش هفته برای بازگشت آن‌ها صبر می‌کنیم.
...
|:--|:--|--:|:--|
| **پلی‌لیست اصلی** | برای هر شبکه فقط یک لینک پخش می‌خواهید. | ۲۰۱ شبکه | [باز کردن پلی‌لیست اصلی](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/fa/iran.m3u) |
| **پلی‌لیست پشتیبان** | لینک‌های جایگزین هر شبکه را هم می‌خواهید. | ۶۲۴ استریم | [باز کردن پلی‌لیست پشتیبان](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/fa/iran-all-streams.m3u) |
| **نسخه سازگار با تلویزیون هوشمند** | تصویر شبکه نمایش داده می‌شود، اما پخش ادامه پیدا نمی‌کند. | ۱۲۸ شبکه | [باز کردن نسخه سازگار](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/fa/iran-compat.m3u) |

**پلی‌لیست اصلی** شامل همه شبکه‌هاست. در حال حاضر، ۳ شبکه از این فهرست دوباره بررسی می‌شوند. شبکه‌های دارای نشان `[IR]` فقط با آی‌پی ایران در دسترس هستند.
...
| ماهواره‌ای · خبر و امور جاری | ۲۹ | [باز کردن پلی‌لیست](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/categories/sat-news.m3u) |
...
| ماهواره‌ای · مستند، فرهنگ و سبک زندگی | ۵ | [باز کردن پلی‌لیست](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/categories/sat-documentary.m3u) |
...
| مذهبی · مسیحی | ۱۰ | [باز کردن پلی‌لیست](https://raw.githubusercontent.com/shayanline/iptv-iran/main/playlists/categories/religious-christian.m3u) |
...
اگر کیفیت یک شبکه قابل تشخیص باشد، مقدار `tvg-quality` آن با یکی از گزینه‌های `SD`، `HD`، `FHD` یا `4K` ثبت می‌شود. وضعیت فعلی شامل ۱ شبکه با کیفیت 4K، ۵۶ شبکه با کیفیت FHD، ۵۱ شبکه با کیفیت HD، ۸۷ شبکه با کیفیت SD، ۶ شبکه با کیفیت نامشخص است.
...
اگر تصویر یک شبکه فقط برای لحظه‌ای نمایش داده می‌شود، یا تلویزیون خطای مانیفست HLS نشان می‌دهد، **نسخه سازگار با تلویزیون هوشمند** را امتحان کنید. این پلی‌لیست شامل ۱۲۸ شبکه است که ساختار استریم آن‌ها روی پخش‌کننده‌های محدود بهتر کار می‌کند.
...
نسخه فعلی شامل ۲۰۱ شبکه در ۱۳ دسته‌بندی است. این شبکه‌ها از میان ۶۲۴ استریم فعال و استریم‌هایی که موقتاً دوباره بررسی می‌شوند انتخاب شده‌اند. برای دیدن جزئیات، [فهرست فارسی شبکه‌ها](CHANNELS.fa.md) یا [فهرست انگلیسی شبکه‌ها](CHANNELS.md) را باز کنید.
