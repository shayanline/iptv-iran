...
def category_playlists(channels, lang):
    ...
    rows = ["| " + (" | ".join(["Category", "Channels", "Playlist"]) if lang == "en"
                    else " | ".join(["دسته‌بندی", "تعداد شبکه‌ها", "پلی‌لیست"])) + " |",
            "|:--|--:|:--|"]
    for cid, en, fa, _ in taxonomy.CATEGORIES:
        ...
        link = "Open playlist" if lang == "en" else "باز کردن پلی‌لیست"
        rows.append(f"| {en if lang == 'en' else fa} | {count} | [{link}]({RAW}/categories/{cid}.m3u) |")
    return "\n".join(rows)
...
def quality_summary(channels, lang):
    ...
    labels_fa = {"4K": "4K", "FHD": "FHD", "HD": "HD", "SD": "SD",
                 "unknown": "نامشخص"}
    parts = []
    for tag in ["4K", "FHD", "HD", "SD", "unknown"]:
        if buckets.get(tag):
            if lang == "en":
                ...
                parts.append(f"{fa_digits(len(buckets[tag]))} شبکه با کیفیت {labels_fa[tag]}")
    return "، ".join(parts) if lang == "fa" else ", ".join(parts)


def render_catalog(channels, lang):
    total = len(channels)
    categories = len({channel["category"] for channel in channels})
    if lang == "en":
        return f"""# Channel catalogue

This catalogue contains {total} channels in {categories} categories. The playlist links are generated from the same published channel data.
...
این فهرست شامل {fa_digits(total)} شبکه در {fa_digits(categories)} دسته است. لینک‌های پلی‌لیست از همان داده منتشرشده ساخته می‌شوند.
...
def render(channels, checked_at):
    total = len(channels)
    rechecking = sum(1 for channel in channels if channel["reach"] == "failing")
    checked = f"{dt.datetime.fromisoformat(checked_at):%d %B %Y}"
    streams = sum(1 for c in channels for s in c["streams"] if not s.get("unlisted_only"))
    categories = len({c["category"] for c in channels})
    with_logo = sum(1 for c in channels if c["logo"])
    compatible = sum(1 for c in channels if c.get("compat_public"))

    english = f"""<a id="english"></a>

# IPTV Iran

IPTV Iran provides {total} curated Iranian and Persian language television channels as remote M3U playlists. Add a playlist URL to a compatible player, with no project account or project software to install.

[![Refresh playlists](https://github.com/{REPO}/actions/workflows/refresh.yml/badge.svg)](https://github.com/{REPO}/actions/workflows/refresh.yml) ![Channels](https://img.shields.io/badge/channels-{total}-1f6feb) ![Checked](https://img.shields.io/badge/last%20checked-{checked.replace(' ', '%20')}-2da44e) [![License: MIT](https://img.shields.io/github/license/{REPO})](LICENSE)

> **Status:** This community maintained project checks streams twice each month. Availability can change between checks, and {rechecking} channels in the main playlist are currently being rechecked during the six week grace period.
...
|:--|:--|--:|:--|
| **Main playlist** | You want one entry for every channel. | {total} channels | [Open the main playlist]({RAW}/iran.m3u) |
| **With backups** | You want alternate entries for a channel. | {streams} streams | [Open the backup playlist]({RAW}/iran-all-streams.m3u) |
| **Smart TV safe** | Your player shows one frame and then stops. | {compatible} channels | [Open the compatibility playlist]({RAW}/iran-compat.m3u) |

The **Main playlist** contains every channel, including {rechecking} channels that are being rechecked during the grace period. Entries marked `[IR]` require an Iranian connection.

The backup playlist lists alternate entries separately. It does not switch streams automatically.

### Recommended playlist

For most viewers, [open the main playlist]({RAW}/iran.m3u).
...
{details("Use one category", category_playlists(channels, "en"))}
...
|:--|:--|:--|
| Both languages | `IRIB TV1 \\| شبکه یک` | [Open the bilingual playlist]({RAW}/iran.m3u) |
| English | `IRIB TV1` | [Open the English playlist]({RAW}/en/iran.m3u) |
| Persian | `شبکه یک` | [Open the Persian playlist]({RAW}/fa/iran.m3u) |

Each entry can include a measured `tvg-quality` value of `SD`, `HD`, `FHD`, or `4K`. The current channel mix is {quality_summary(channels, "en")}.

The project stores logos for {with_logo} channels in this repository to avoid dependence on logo hosts that may block some regions. Programme data is currently unavailable because the previous external guide feed was retired. Players may still accept a separately configured guide.

<a id="smart-tv-playback"></a>

## Smart TV playback

Use the **Smart TV safe** playlist when a channel displays one frame and then stops, or when a television reports an HLS manifest error. It contains the {compatible} channels whose public streams avoid manifest shapes known to break limited players.
...
The current publication contains {total} channels in {categories} categories from {streams} tracked working or grace period streams. Read the [English channel catalogue](CHANNELS.md) or the [Persian channel catalogue](CHANNELS.fa.md).
...
Read [SUPPORT.md](SUPPORT.md) before [opening a public issue](https://github.com/{REPO}/issues). Include the channel, playlist, player, device, country, test time, and VLC comparison so another person can reproduce the problem.
...
Rights holders can request removal through a [public issue](https://github.com/{REPO}/issues). Include the channel name, the affected address, and evidence that you represent the rights holder.
...
این پروژه {fa_digits(total)} شبکه تلویزیونی ایرانی و فارسی‌زبان را در قالب پلی‌لیست‌های M3U ارائه می‌کند. لینک یکی از پلی‌لیست‌ها را در یک برنامه پخش سازگار وارد کنید تا شبکه‌ها در دسترس شما قرار بگیرند. استفاده از این پلی‌لیست‌ها به حساب کاربری یا نصب نرم‌افزار جداگانه‌ای از طرف پروژه نیاز ندارد.

> **وضعیت پروژه:** این پروژه به‌صورت داوطلبانه نگهداری می‌شود و لینک‌های پخش ماهی دو بار بررسی می‌شوند. ممکن است وضعیت یک شبکه در فاصله میان دو بررسی تغییر کند. در حال حاضر، {fa_digits(rechecking)} شبکه از پلی‌لیست اصلی دوباره بررسی می‌شوند و تا شش هفته برای بازگشت آن‌ها صبر می‌کنیم.
...
|:--|:--|--:|:--|
| **پلی‌لیست اصلی** | برای هر شبکه فقط یک لینک پخش می‌خواهید. | {fa_digits(total)} شبکه | [باز کردن پلی‌لیست اصلی]({RAW}/fa/iran.m3u) |
| **پلی‌لیست پشتیبان** | لینک‌های جایگزین هر شبکه را هم می‌خواهید. | {fa_digits(streams)} استریم | [باز کردن پلی‌لیست پشتیبان]({RAW}/fa/iran-all-streams.m3u) |
| **نسخه سازگار با تلویزیون هوشمند** | تصویر شبکه نمایش داده می‌شود، اما پخش ادامه پیدا نمی‌کند. | {fa_digits(compatible)} شبکه | [باز کردن نسخه سازگار]({RAW}/fa/iran-compat.m3u) |

**پلی‌لیست اصلی** شامل همه شبکه‌هاست. در حال حاضر، {fa_digits(rechecking)} شبکه از این فهرست دوباره بررسی می‌شوند. شبکه‌های دارای نشان `[IR]` فقط با آی‌پی ایران در دسترس هستند.
...
برای بیشتر کاربران، [پلی‌لیست اصلی]({RAW}/fa/iran.m3u) انتخاب مناسبی است.
...
{details("پلی‌لیست‌های هر دسته‌بندی", category_playlists(channels, "fa"))}
...
|:--|:--|:--|
| دوزبانه | `IRIB TV1 \\| شبکه یک` | [باز کردن نسخه دوزبانه]({RAW}/iran.m3u) |
| انگلیسی | `IRIB TV1` | [باز کردن نسخه انگلیسی]({RAW}/en/iran.m3u) |
| فارسی | `شبکه یک` | [باز کردن نسخه فارسی]({RAW}/fa/iran.m3u) |

اگر کیفیت یک شبکه قابل تشخیص باشد، مقدار `tvg-quality` آن با یکی از گزینه‌های `SD`، `HD`، `FHD` یا `4K` ثبت می‌شود. وضعیت فعلی شامل {quality_summary(channels, "fa")} است.

لوگوی {fa_digits(with_logo)} شبکه داخل همین مخزن قرار دارد تا نمایش لوگوها به سایت‌هایی وابسته نباشد که ممکن است در بعضی کشورها باز نشوند. اطلاعات راهنمای برنامه‌ها یا EPG فعلاً در پلی‌لیست‌ها وجود ندارد، چون منبع قبلی دیگر فعال نیست. اگر منبع معتبر دیگری دارید، می‌توانید آن را جداگانه در برنامه پخش خود وارد کنید.

<a id="fa-smart-tv"></a>

## پخش در تلویزیون هوشمند

اگر تصویر یک شبکه فقط برای لحظه‌ای نمایش داده می‌شود، یا تلویزیون خطای مانیفست HLS نشان می‌دهد، **نسخه سازگار با تلویزیون هوشمند** را امتحان کنید. این پلی‌لیست شامل {fa_digits(compatible)} شبکه است که ساختار استریم آن‌ها روی پخش‌کننده‌های محدود بهتر کار می‌کند.
...
نسخه فعلی شامل {fa_digits(total)} شبکه در {fa_digits(categories)} دسته‌بندی است. این شبکه‌ها از میان {fa_digits(streams)} استریم فعال و استریم‌هایی که موقتاً دوباره بررسی می‌شوند انتخاب شده‌اند. برای دیدن جزئیات، [فهرست فارسی شبکه‌ها](CHANNELS.fa.md) یا [فهرست انگلیسی شبکه‌ها](CHANNELS.md) را باز کنید.
...
پیش از [ثبت مشکل در GitHub](https://github.com/{REPO}/issues)، راهنمای [پشتیبانی](SUPPORT.md) را بخوانید. در گزارش خود نام شبکه، پلی‌لیست، برنامه پخش، مدل دستگاه، کشور، زمان آزمایش و نتیجه مقایسه با VLC را بنویسید تا دیگران بتوانند مشکل را تکرار کنند.
...
دارندگان حق پخش می‌توانند از طریق [ثبت درخواست در GitHub](https://github.com/{REPO}/issues) حذف یک لینک را درخواست کنند. نام شبکه، لینک موردنظر و مدرکی را ارائه کنید که نشان دهد از طرف دارنده حق پخش آن شبکه درخواست می‌دهید.
