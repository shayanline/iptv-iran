...
def blocked_hosts(curated):
    private = os.getenv("PRIVATE_BLOCKED_HOSTS", "").split(",")
    return tuple(host.strip().lower() for host in [*curated.get("blocked_hosts", []), *private]
                 if host.strip())
...
def harvest():
    ...
    for stream in curated.get("streams", []):
        cands.add(stream["url"], "curated", stream.get("channel"), stream.get("title"),
                  ...
                  unlisted_only=stream.get("unlisted_only"))
    ...
    for rec in cands.by_key.values():
        host = (urllib.parse.urlparse(rec["url"]).hostname or "").lower()
        ...
        if cid:
            ...
            rec["db"] = {
                ...
                "closed": None if cid in local else channel.get("closed"),
