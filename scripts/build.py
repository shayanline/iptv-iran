...
def compatible_stream(stream):
    return stream["state"] == "ok" and not stream["hazards"] and not stream["defects"]
...
def collect():
    ...
    for record in candidates:
        ...
        channel["streams"].append({
            ...
            "unlisted_only": bool(record.get("unlisted_only")),
    ...
    for channel in channels.values():
        ...
        public_streams = [s for s in channel["streams"] if not s["unlisted_only"]]
        eligible = public_streams or channel["streams"]
        channel["public"] = bool(public_streams)
        best = eligible[0]
        ...
        states = {s["state"] for s in eligible}
        if "ok" in states:
        ...
        channel["height"] = max(s["height"] for s in eligible)
        channel["resolution"] = next((s["resolution"] for s in eligible
        ...
        channel["compat"] = next((s for s in channel["streams"]
                                  if compatible_stream(s)), None)
        # What the advertised compatibility playlist may use: no worker or hidden proxy involved.
        channel["compat_public"] = next((s for s in public_streams
                                         if compatible_stream(s)), None)
    ...
    ordered = sorted(channels.values(),
                     key=lambda c: (taxonomy.ORDER.get(c["category"], 99),
                                    ...
                                    c["name_en"].lower()))
    log(f"{len(ordered)} channels usable, {sum(c['public'] for c in ordered)} public "
...
def write_playlist(path, channels, note, lang="both", all_streams=False, use_compat=False,
    ...
    for channel in channels:
        if all_streams:
            chosen = [s for s in channel["streams"] if not s.get("unlisted_only")]
        else:
...
def build_playlists(channels):
    ...
    # `compat_public` never routes through the worker or an unlisted source, so the
    # playlists named in the README cannot generate traffic against someone else's budget.
    public = [c for c in channels if c.get("public", True)]
    compatible = [c for c in public if c.get("compat_public")]

    for lang, folder in (("both", PLAYLISTS), ("en", PLAYLISTS / "en"), ("fa", PLAYLISTS / "fa")):
        write_playlist(folder / "iran.m3u", public,
                       "Every channel, one stream each. [IR] needs an Iranian IP address.", lang)
        write_playlist(folder / "iran-all-streams.m3u", public,
                       "Working and recently working streams, backups included, best first.", lang,
                       all_streams=True)
    ...
    for cid, *_ in taxonomy.CATEGORIES:
        members = [c for c in public if c["category"] == cid]
        if members:
...
def build_readme(channels):
    from readme import render, render_catalog
    channels = [c for c in channels if c.get("public", True)]
    checked_at = read_json(DATA / "status.json", {})["generated_at"]
    text = render(channels, checked_at)
    english_catalog = render_catalog(channels, "en")
    persian_catalog = render_catalog(channels, "fa")
    (HERE / "README.md").write_text(text, encoding="utf-8")
    (HERE / "CHANNELS.md").write_text(english_catalog, encoding="utf-8")
    (HERE / "CHANNELS.fa.md").write_text(persian_catalog, encoding="utf-8")
    log(f"  README.md: {len(text.splitlines())} lines")
    log(f"  CHANNELS.md: {len(english_catalog.splitlines())} lines")
    log(f"  CHANNELS.fa.md: {len(persian_catalog.splitlines())} lines")
